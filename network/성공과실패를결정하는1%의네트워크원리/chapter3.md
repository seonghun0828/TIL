...
라우터는 주소 변환과 패킷 필터링 기능의 추가적인 부가 기능을 가지고 있다.   
사내 네트워크에서는 사내 기기에 할당하는 주소가 다른 회사와 중복되어도 상관 없다.   
때문에 사내 네트워크에서는 프라이빗 주소(private adress)를, 밖에서는 글로벌 주소(global adress)를 사용한다.   
    
덕분에 주소를 절약할 수는 있지만 사내 네트워크가 인터넷에 연결되어 있는 경우에는   
사내 네트워크와 인터넷 어딘가에 같은 주소가 있게 되어 패킷을 정확히 운반할 수 없게 된다.   
그러므로 주소 변환을 사용한다.
    
    2. 주소 변환의 기본 동작   
주소 변환의 구조는 패킷을 중계할 때 IP 헤더에 기재된 IP 주소와 포트 번호를 바꿔쓰는 것이다.   
주소 변환 장치(라우터, 방화벽 등)에는 주소와 포트의 '대응표'가 존재하는데 이것을 통해 주소를 변환한다.   
    
    3. 포트 번호를 바꿔쓰는 이유   
글로벌 주소와 프라이빗 주소가 1대 1로 대응되면 비효율적이므로 사내 네트워크의 주소는 한 개로 설정한다.   
대신 포트 번호를 제각각 부여하면 다수의 글로벌 주소에 프라이빗 주소에 대응시킬 수 있다.
    
    4. 인터넷에서 회사로 액세스한다   
사내에서 인터넷으로 액세스하는 패킷을 중계할 때는 대응표에 송신처의 프라이빗 주소와 포트 번호가 등록되어 있지 않아도 가능하다.    
바꿔 쓰는 글로벌 주소는 이미 주소 변환 장치에 할당되어 있고, 포트 번호는 적당히 비어있는 것을 사용하면 되기 때문이다.   
그러나 반대의 경우, 인터넷에서 사내로 패킷을 중계할 때는 대응표에 기록이 없으면 주소 변환이 불가능할 것이다.   
   
이것은 부정 침입 효과를 가지고 있다. 프라이빗 주소를 주소 변환 장치에 수동으로 등록해 두면 주소 변환은 가능하다.
    
    5. 라우터의 패킷 필터링 기능
패킷 필터링 기능은 사전에 조건을 설정하고 패킷을 중계할 때 MAC, IP, TCP 헤더를 조사해 조건에 따라 패킷을 중계 혹은 폐기하는 것이다.   
